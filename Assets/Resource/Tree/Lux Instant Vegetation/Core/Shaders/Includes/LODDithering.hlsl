...
#endif
void LuxLODDitheringTransition_float(float3 viewDirWS, float4 screenPos, out float multiplyAlpha)
{
...
void LuxLODDitheringTransition_half(float3 viewDirWS, float4 screenPos, out half halfAlpha)
{
#if !defined (SHADER_API_GLES) && !defined(SHADER_STAGE_RAY_TRACING)
    float p = GenerateHashedRandomFloat(ComputeFadeMaskSeed(viewDirWS, screenPos.xy));
    float f = unity_LODFade.x - CopySign(p, unity_LODFade.x);
